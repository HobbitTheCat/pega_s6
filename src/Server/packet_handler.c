...
int handle_packet_main(server_t* server, server_conn_t* conn, const uint8_t packet_type, const uint8_t* payload, const uint16_t payload_length) {
    ...
    if (conn->state == CONN_STATE_INIT) {
        log_bus_push_message(server->log_bus, 0, LOG_WARN, "Wrong packet order");
        return send_error_packet(server, conn, 0x01, "Wrong packet order");
    }
    if (packet_type == PKT_REGISTER) {
        if (conn->player != NULL) {
            log_bus_push_message(server->log_bus, 0, LOG_WARN, "User already registered");
            return send_error_packet(server, conn, 0x02, "User already registered");
        }
    ...
    if (conn->player == NULL) {
        log_bus_push_message(server->log_bus, 0, LOG_WARN, "User is not registered");
    ...
    switch (packet_type) {
        ...
        case PKT_SESSION_CREATE:
            if (player->session_id != 0) {
                log_bus_push_message(server->log_bus, 0, LOG_WARN, "Session already exist");
                send_error_packet(server, conn, 0x04, "Session already exist"); break;
            }
            const int session_id = handle_session_create_packet(server, conn, payload, payload_length);
            if (session_id < 0) {
                log_bus_push_message(server->log_bus, 0, LOG_ERROR, "Session creation failed");
                send_error_packet(server, conn, 0x05, "Session creation failed");
            }
        ...
        case PKT_SESSION_JOIN:
            ...
            if (!fd_map_get(server->registered_sessions, (int)id)) {
                ...
                send_error_packet(server, conn, 0x05, "Session not found"); break;
            }
